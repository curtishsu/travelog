    ...
    <>
      ...
      {isLightboxOpen && activePhoto ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur">
          <button
            type="button"
            className="absolute right-6 top-6 z-30 rounded-full border border-slate-700 bg-slate-900/80 p-2 text-slate-200 hover:text-white"
            onClick={closeLightbox}
            aria-label="Close lightbox"
          >
            <X className="h-5 w-5" />
          </button>
          <div className="group relative mx-auto h-[60vh] max-h-[80vh] w-full max-w-4xl overflow-hidden rounded-3xl border border-slate-700 bg-slate-900 shadow-2xl">
            <Image
            ...
            {photos.length > 1 ? (
              <>
                ...
                >
                  <span className="pointer-events-none absolute left-4 top-1/2 hidden -translate-y-1/2 rounded-full border border-slate-700 bg-slate-900/80 p-3 text-slate-200 transition hover:text-white group-hover:block md:block md:opacity-0 md:group-hover:opacity-100">
                ...
                >
                  <span className="pointer-events-none absolute right-4 top-1/2 hidden -translate-y-1/2 rounded-full border border-slate-700 bg-slate-900/80 p-3 text-slate-200 transition hover:text-white group-hover:block md:block md:opacity-0 md:group-hover:opacity-100">
